...
from FreeMEF.models import lr_scheduler as lr_scheduler
...
class HDR_Model(SRModel):
    ...
    def __init__(self, opt):
        super(HDR_Model, self).__init__(opt)
        if self.is_train:
    ...
    def feed_data(self, data):
        ...
        if 'others' in data:
            self.others = data['others'].to(self.device)
        else:
            self.others = None
        if 'others_count' in data:
            self.others_count = data['others_count']
        else:
    ...
    def nondist_validation(self, dataloader, current_iter, tb_logger, save_img):
        ...
        self.is_train = False
        super(HDR_S1Model, self).nondist_validation(dataloader, current_iter, tb_logger, save_img)
        self.is_train = True
    ...
    def test(self):
        if hasattr(self, 'net_g_ema'):
            self.net_g_ema.eval()
            with torch.no_grad():
                self.output = self.net_g_ema(self.lq, self.others, self.others_count)
        else:
            self.net_g.eval()
            with torch.no_grad():
                self.output = self.net_g(self.lq, self.others, self.others_count)
            self.net_g.train()

    def optimize_parameters(self, current_iter):
        self.optimizer_g.zero_grad()
        if self.others_count is not None:
            self.output = self.net_g(self.lq, self.others, self.others_count)
        else:
