...
def train_pipeline(root_path):
    ...
    for epoch in range(start_epoch, total_epochs + 1):
        ...
        while train_data is not None:
            ...
            lq = train_data['lq']
            gt = train_data['gt']
            others = train_data.get('others', None)
            others_count = train_data.get('others_count', None)
            ...
            if mini_batch_size < batch_size:
                ...
                if others_count is not None:
                    others_count = others_count[indices]
            ...
            if mini_gt_size < gt_size:
                ...
                if others is not None and others.numel() > 0:
                    others = others[:,:,:,x0:x1,y0:y1]
            ###-------------------------------------------


            # training
            feed_dict = {'lq': lq, 'gt':gt}
            if others is not None:
                feed_dict['others'] = others
            if others_count is not None:
                feed_dict['others_count'] = others_count
